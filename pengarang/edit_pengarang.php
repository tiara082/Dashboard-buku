...
<body>
    ...
    <section class="content">
        ...
        <?php
            ...
            $sql="SELECT * FROM perpus.pengarang where id_pengarang ='$id_pengarang'";
            $result=$conn->query($sql);
            $row=$result->fetch_assoc();


            $nama = $row['nama_pengarang'];
            $email = $row['email'];
            $telp = $row['telp'];
            $alamat = $row['alamat'];
        ...
        <form action="#" method="post" >
            <table  width="100%" class="table table-borderless" cellpadding ='10'>
                ...
                <tr>
                    ...
                    <td><input type="submit" name="simpan" value="Simpan" class="btn btn-success rounded-0 px-4 submit">
                        <a href="http://localhost/Dashboard-buku/pengarang/index.php" class="btn btn-secondary rounded-0 px-4">Kembali</a>
                    </td>
               </tr>
            </table>
            <?php
            if(isset($_POST['simpan'])){
                ...
                $sql="update perpus.pengarang set nama_pengarang='$nama',email=' $email',telp='$telp',alamat='$alamat' where pengarang.id_pengarang = '$id_pengarang'";
                    if ($conn->query($sql) === TRUE ) {
                        echo "<script type='text/javascript'>
                                    alert('Data berhasil diubah ');
                                    window.location='http://localhost/Dashboard-buku/pengarang/index.php';
                                 </script>";
